...
- (void)layoutSubviews {
    ...
    CGFloat contentHeight = CGRectGetHeight(self.textView.frame) + verticalMargin + buttonHeight * buttonRows
                            + buttonMargin * (buttonRows - 1);

    CGSize contentViewSize = CGSizeMake(contentWidth, contentHeight);

    CGRect rect = self.contentView.frame;
    rect.origin = CGPointMake(CGRectGetWidth(self.bounds) / 2 - contentWidth / 2,
                              CGRectGetHeight(self.bounds) / 2 - contentHeight / 2);
    ...
    for (UIButton *button in self.buttons) {
        button.frame = CGRectMake(
            buttonPosition.x, buttonPosition.y, CGRectGetWidth(button.bounds), CGRectGetHeight(button.bounds));
        column += 1;
