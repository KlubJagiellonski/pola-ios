...
- (void)sendImagesWithData:(BPCapturedImagesData *)imagesData
    ...
    void (^onCompletion)(NSError *) = ^void(NSError *error) {
        ...
        if (error) {
            [strongSelf.imageManager removeImagesDataForCaptureSessionTimestamp:timestamp
                                                                     imageCount:imagesData.filesCount.intValue];
        }
    ...
    void (^block)() = ^{
        ...
        if (error) {
            [dispatchQueue addOperationWithBlock:^{
                progress([[BPCapturedImageResult alloc] initWithState:CAPTURED_IMAGE_STATE_ADDING
                                                   capturedImagesData:imagesData
                                                           imageIndex:-1],
                         error);
                onCompletion(error);
            }];
        ...
        NSArray<NSData *> *imageDataArray =
            [strongSelf.imageManager retrieveImagesDataForCaptureSessionTimestamp:timestamp imageCount:imageCount];

        __block int amountOfFinishedUploads = 0;
        for (int i = 0; i < imageCount; i++) {
            [strongSelf
                uploadImageData:imageDataArray[i]
                          toUrl:signedRequestArray[i]
                       mimeType:imagesData.mimeType
                     completion:^(NSError *imageUploadError) {
                         [dispatchQueue addOperationWithBlock:^{
                             if (imageUploadError) {
                                 progress([[BPCapturedImageResult alloc] initWithState:CAPTURED_IMAGE_STATE_UPLOADING
                                                                    capturedImagesData:imagesData
                                                                            imageIndex:i],
                                          imageUploadError);
                             } else {
                                 progress([[BPCapturedImageResult alloc] initWithState:CAPTURED_IMAGE_STATE_FINISHED
                                                                    capturedImagesData:imagesData
                                                                            imageIndex:i],
                                          imageUploadError);
                             }
