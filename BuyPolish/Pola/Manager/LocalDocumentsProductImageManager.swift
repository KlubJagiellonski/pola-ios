...
final class LocalDocumentsProductImageManager: ProductImageManager {
    ...
    func saveImage(_ image: UIImage, for key: ReportProblemReason, index _: Int) -> Bool {
...
private enum ProductImageSize: String {
    case big
    case thumbnail
...
private extension ReportProblemReason {
    var directoryName: String {
        switch self {
        case .general:
            return "general"
        case let .product(productId, _):
            return "\(productId)"
        }
    }
...
private extension UIImage {
    func data(for imageSize: ProductImageSize) -> Data? {
        scaled(toWidth: imageSize.widht).pngData()
    }

    func scaled(to imageSize: ProductImageSize) -> UIImage {
        return scaled(toWidth: imageSize.widht)
    }
}
